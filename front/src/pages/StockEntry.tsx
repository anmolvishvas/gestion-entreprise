...
export default function StockEntry() {
  ...
  const { stockItems, stockMovements, addStockMovement } = useAppContext();
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-600">Ajoutez des articles à votre inventaire</p>
      </div>
              ...
              <form onSubmit={handleSubmitExistingItem}>
                <div className="mb-6">
                  <div className="flex items-center mb-1">
                    ...
                    <h2 className="text-xl font-semibold text-gray-900">Entrée d'un article existant</h2>
                  </div>
                  <p className="text-gray-600 text-sm">Ajoutez du stock à un article existant</p>
                </div>
                ...
                <div className="mb-4">
                  ...
                  >
                    ...
                    <optgroup label="Cotona">
                      {filteredItems
                        .filter(item => item.location === 'Cotona')
                        .map(item => (
                          <option key={item.id} value={item.id}>
                            {item.name} ({item.stockRestant} en stock)
                          </option>
                        ))}
                    </optgroup>
                    ...
                    <optgroup label="Maison">
                      {filteredItems
                        .filter(item => item.location === 'Maison')
                        .map(item => (
                          <option key={item.id} value={item.id}>
                    ...
                    <optgroup label="Avishay">
                      {filteredItems
                        .filter(item => item.location === 'Avishay')
                        .map(item => (
                          <option key={item.id} value={item.id}>
                            {item.name} ({item.stockRestant || item.stockInitial} en stock)
                          </option>
                    ...
                    <optgroup label="Avenir">
                      {filteredItems
                        .filter(item => item.location === 'Avenir')
                        .map(item => (
                          <option key={item.id} value={item.id}>
                            {item.name} ({item.stockRestant || item.stockInitial} en stock)
                          </option>
              ...
              <form onSubmit={handleSubmitNewItem}>
                <div className="mb-6">
                  <div className="flex items-center mb-1">
                    ...
                    <h2 className="text-xl font-semibold text-gray-900">Ajouter un nouvel article</h2>
                  </div>
                  <p className="text-gray-600 text-sm">Créez un nouvel article dans l'inventaire</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="input w-full"
                      value={newItemUnit}
                      onChange={(e) => setNewItemUnit(e.target.value as 'piece' | 'unite')}
                    >
        ...
        <div className="md:col-span-1">
          <div className="bg-white p-6 rounded-lg shadow mb-6">
            ...
            {!newItemMode ? (
              <div className="space-y-3 text-sm text-gray-600">
                ...
                <p>2. Sélectionnez l'article dans la liste déroulante</p>
                <p>3. Entrez la quantité à ajouter au stock</p>
                <p>4. Ajoutez éventuellement des notes</p>
                <p>5. Cliquez sur "Ajouter au stock" pour valider</p>
              </div>
            ) : (
              <div className="space-y-3 text-sm text-gray-600">
                <p>1. Entrez le nom du nouvel article</p>
                <p>2. Sélectionnez l'emplacement où l'article sera stocké</p>
                <p>3. Choisissez le type de l'article</p>
                <p>4. Définissez la quantité initiale en stock</p>
                <p>5. Sélectionnez l'unité de mesure (pièce ou unité)</p>
                <p>6. Cliquez sur "Créer et ajouter" pour valider</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
            <div className="flex items-center mb-3">
              ...
              <h3 className="font-semibold text-indigo-900">Conseil</h3>
            </div>
            <p className="text-sm text-indigo-700">
              Pour un nouvel arrivage de produits existants, utilisez "Article existant". 
              Pour ajouter un nouveau type de produit à votre catalogue, utilisez "Nouvel article".
            </p>
          </div>
        </div>
      </div>
